# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/Infopanel/SoftcamPanel.py
...
class ShowSoftcamPackages(Screen):
    ...
    def __init__(self, session, args = None):
        ...
        self['actions'] = ActionMap(['OkCancelActions', 'DirectionActions', 'ColorActions'], {'red': self.exit,
         'ok': self.go,
         'cancel': self.exit,
         'green': self.startupdateList}, -1)
        self.list = []
        self.statuslist = []
        ...
        if status:
            ...
            if status == 'list':
                ...
                self.statuslist.append((_('Package list'),
                 '',
                 _('Getting Softcam list. Please wait...'),
                 '',
                 statuspng,
                 divpng))
                self['list'].setList(self.statuslist)
